...
function calendarRegion() {
  return query.region("미팅 날짜 달력");
}

function meetingDateButton() {
  return query.within(calendarRegion(), query.button(FORMATTED_MEETING_DATE));
}
...
describe("MeetingBookingForm", () => {
  it("미팅 날짜 선택기에서 달력을 열 수 있다", async () => {
    await runSiheom(
      ...
      assertions.visible(calendarRegion()),
    );
  });
  ...
  it("미팅 날짜를 선택하고 예약할 수 있다", async () => {
    await runSiheom(
      setup(),
      actions.click(query.button("미팅 날짜")),
      actions.click(meetingDateButton()),
