...
import { actions, assertions, given, query, runSiheom } from "@siheom/qwik";
...
- `given.render`가 Counter를 `@noma.to/qwik-testing-library`의 `render`로 실제 브라우저에 마운트합니다.
...
`useContextProvider`로 값을 내려줘야 하면, 감싸는 컴포넌트를 만들어 `given.render`를 확장합니다.
