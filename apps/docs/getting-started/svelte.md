...
실제 브라우저에서 첫 시험을 작성하고, 실패 리포트를 읽고, context로 컴포넌트를 감싸는 방법까지 이 페이지에서 다룹니다.
...
```bash
yarn add -D vitest @vitest/browser playwright @sveltejs/vite-plugin-svelte
yarn playwright install chromium
yarn add @siheom/svelte svelte @testing-library/svelte @testing-library/jest-dom @testing-library/user-event
```
...
- `given.render`가 Counter를 `@testing-library/svelte`의 `render`로 실제 브라우저에 마운트합니다.
...
Svelte의 context API로 값을 주입해야 하면 `given.render`를 확장합니다. `@testing-library/svelte`의 `render`는 두 번째 인자로 `context` Map을 받습니다.
