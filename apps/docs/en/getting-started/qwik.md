...
- `given.render` mounts Counter in a real browser via `@noma.to/qwik-testing-library`'s `render`.
...
If a component reads values from `useContextProvider`, wrap it in a provider component and extend `given.render`.
